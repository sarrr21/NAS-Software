  ...
  return (
    <div className="p-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {items.map((item, index) => (
            <div key={index} className="border rounded-lg shadow-lg p-4">
              <img src={item.photo} alt={item.title} className="w-full h-48 object-cover mb-4" />
              <h2 className="text-xl font-bold mb-2">{item.title}</h2>
              <p className="text-gray-700">{item.description}</p>
              <button
                onClick={() => handleViewClick(item)}
                className="text-yellow-500 py-2 px-6 rounded-lg border"
              >
                View
              </button>
            </div>
      ...
      <div className="flex justify-center">
        <button className="items-center rounded-lg px-8 py-4 border">View All Talent</button>
      </div>
